...
public class CMDList extends Command{
	...
	public CMDList(){
		super("list", "List all stream notifications this guild subscribed to", false, new CommandCooldown(CommandCooldown.Type.User, 2000),
			null,
	...
	public void onExecution(CmdArgs cmdArgs, CommandEvent commandEvent, TranslationPackage translationPackage, boolean ackRequired) throws Exception{
		...
		MessageEmbed result = EmbedBuilderFactory
			.getDefaultEmbed(translationPackage.getTranslation(getClass(), "response.success.title"), commandEvent.getEvent().getUser())
			.setDescription(stringBuilder)
			.build();
		commandEvent.getEvent().replyEmbeds(result).queue();
	}
